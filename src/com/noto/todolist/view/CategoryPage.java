...
public class CategoryPage extends JFrame {
    ...
    public CategoryPage(int userId) {
        this.userId = userId;
        this.dbManager = DatabaseManager.getInstance();
        
        setTitle("Note Categories - User " + userId);
        setSize(900, 600);
...
//        notesHeaderPanel.add(addNoteBtn, BorderLayout.EAST);
    ...
    private void handleAddCategory() {
        ...
        if (categoryName != null && !categoryName.trim().isEmpty()) {
            ...
            // Choose color
            Color color = JColorChooser.showDialog(this, "Choose Category Color", Color.WHITE);
            if (color == null) {
                color = Color.WHITE;
            }
    ...
    private void handleEditCategory(String categoryName) {
        ...
        String newName = JOptionPane.showInputDialog(this, "Enter new category name:", categoryName);
        if (newName != null && !newName.trim().isEmpty()) {
            ...
            if (!newName.equals(categoryName) && categoryIds.containsKey(newName)) {
                JOptionPane.showMessageDialog(this, "Category name already exists.", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            
            // Choose color
            Color currentColor = categoryColors.getOrDefault(categoryName, Color.WHITE);
            Color newColor = JColorChooser.showDialog(this, "Choose Category Color", currentColor);
            if (newColor == null) {
